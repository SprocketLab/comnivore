...
class ColoredMNIST_Candidate_Set(DomainBed_Candidate_Set):
    def __init__(self, reshape_size, batch_size):
        self.dataset = vars(datasets)["ColoredMNIST"](domainbed_const.DATA_DIR,
                                               domainbed_const.TEST_ENVS, {'data_augmentation': None})

        super(ColoredMNIST_Candidate_Set, self).__init__(self.dataset)
    ...
    def get_train_dataloader(self, batch_size):
        train_loader = super().get_mnist_train_loader(batch_size)
        return train_loader
